...
def plot_topkth_pair(M, spectral_labels, top_pairs, spectral_matrix, to_plot, kth_pair, axes, axes_pos, n_clusters, matrix_name):
    ...
    worldplot = world.plot(column='cluster',
    ...
    axes[axes_pos[0],axes_pos[1]].text(-190, -90, "Trade Flow (TF) = %.4E" % kth_best[0][0])\
                    #   + "\nCI_vol = " + str(round(kth_best[0][2], 4)) \
                    #   + "\nCI_size = " + str(round(kth_best[0][1], 4)))
    axes[axes_pos[0],axes_pos[1]].set_title(matrix_name)
#    axes[axes_pos[0],axes_pos[1]].axis('off')
    axes[axes_pos[0],axes_pos[1]].get_xaxis().set_visible(False)
    axes[axes_pos[0],axes_pos[1]].get_yaxis().set_visible(False)

    # axes[axes_pos[0],axes_pos[1]].tick_params(
...
def plot_top_clusters():
    """
    """
    for n_clus_vect in [(2,1)]:#,(15,7),(20,10)]:
        runs = 1
        ...
        # for i, cluster_num in enumerate(spectral_labels_Herm_RW):
        #     if cluster_num == 0:
        #         herm_rw_cluster0.append(id_to_country_dic[matrixid_to_id[i]])
        #     if cluster_num == 1:
        #         herm_rw_cluster1.append(id_to_country_dic[matrixid_to_id[i]])

        # # print "cluster 1 from splitting up by net flow:\n", cluster1
        # # print "cluster 2 from splitting up by net flow:\n", cluster2

        # # print "cluster 1 from Herm_RW clustering:\n", herm_rw_cluster0
        # # print "cluster 2 from Herm_RW clustering:\n", herm_rw_cluster1

        # print '\nbelow0_0 len:', len(cluster1)
        # print 'herm_row_cluster0 len', len(herm_rw_cluster0)
        # print 'their intersection:', len(set(herm_rw_cluster0).intersection(set(cluster1)))

        # print '\nbelow0_0 len:', len(cluster1)
        # print 'herm_row_cluster1 len', len(herm_rw_cluster1)
        # print 'their intersection:', len(set(herm_rw_cluster1).intersection(set(cluster1)))

        # print '\nbelow0_1 len:', len(cluster2)
        # print 'herm_row_cluster0 len', len(herm_rw_cluster0)
        # print 'their intersection:', len(set(herm_rw_cluster0).intersection(set(cluster2)))

        # print '\nbelow0_1 len:', len(cluster2)
        # print 'herm_row_cluster1 len', len(herm_rw_cluster1)
        # print 'their intersection:', len(set(herm_rw_cluster1).intersection(set(cluster2)))
...
if __name__ == "__main__":
    ...
    for year_int in tqdm(years):
        ...
        year = str(year_int)
        data_file = './process_comtrade/wood/processed_data_' + year +  '_44.csv'#
