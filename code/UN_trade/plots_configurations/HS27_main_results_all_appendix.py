...
#        axes.legend(handles=legend_elements)
    ...
    axes[axes_pos[0], axes_pos[1]].text(-190, -90, "$\mathsf{TF}^{\mathsf{norm}}$ = %.2E" % kth_best[0][0]\
                    #   + "\nCI_vol = " + str(round(kth_best[0][2], 4))) \
                                        + "\n $\mathsf{TI}^{\mathsf{size}}$ = " + str(round(kth_best[0][1], 2)))
...
def plot_top_clusters():
    ...
    #        plt.title("legend graph")
            ...
            fig.suptitle('$\mathsf{TF}^{\mathsf{norm}}$ top-' + str(-1*top_i) + ' pair' #'   # + to_plot \
                         + '          k=' + str(n_clusters) + '          year = ' + year
                         , fontsize=50)
...
#            plt.savefig('k_2_sorting_algos.pdf')
            ...
            plt.savefig('mineral27_net_trade_flow_all_appendix_k='+str(n_clusters)+'_' + year + '_top_pair=' + str(-1*top_i) + '.png')
            plt.close('all')
...
if __name__ == "__main__":
    years = range(1988, 2019)# + range(2007, 2010)
