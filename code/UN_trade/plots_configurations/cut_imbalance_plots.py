...
def plot_cut_imbalance_plots():
    ...
    for j in range(3):
        if k_list[j] == 2:
            ...
            plt.ylabel("$\text{TF}^{\text{\mathsf{norm}}}$")
    #        plt.xlabel("top pairs")
            ...
            plt.ylabel("$\mathsf{TF}^{\mathsf{norm}}$")
    #        plt.xlabel("top pairs")
            plt.title(str("K = " +  str(k_list[j])))
    ...
    for j in range(3):
        if k_list[j] == 2:
            ...
            plt.bar(y_pos, performance, align='center', color=colors)
            plt.xticks(y_pos, ('Herm', 'Herm_RW', 'DISG_R', 'DISG_L', 'BI_SYM', 'DD_SYM'))
            plt.ylabel("$\mathsf{TI}^{\mathsf{size}}$")
    #        plt.xlabel("top pairs")
            plt.title(str("K = " +  str(k_list[j])))
        else:
            plt.subplot(2, 3, 3+j+1)
...
if __name__ == "__main__":
    ...
    for year_int in tqdm(years):
        ...
        year = str(year_int)
        data_file = './process_comtrade/crude_oil_271019/processed_data_' + year +  '_271019.csv'#
