...
#            cut_imbalance_vol = cut_imbalance * min([vol_1/len(cluster1_idx), vol_2/len(cluster2_idx)])
    ...
    cut_imbalance_CI = cut_imbalance
...
def plot_top_clusters():
    """
    """
    for n_clus_vect in [(20,10)]:#,(15,7),(20,10)]:
        runs = 50
        ...
        M_sparse = degree_sparsifying_sampler(M, Herm, 2, len(country_codes_withdata), 5)

        n = len(country_codes_withdata)
        Herm_sparse = np.ones((n, n), dtype=complex)*(M_sparse - M_sparse.T)*1j
        Herm = normalize(Herm_sparse)
        ...
        print("sparsified time:", end - start)
        ...
        print("unsparsified time:", end - start)
        ...
        #### ------------ check if there is any overlap between the clusters -------------- ####
        n = len(spectral_labels_sort)

        # HERM_RW
        difference_Herm_RW = (np.array(spectral_labels_sort) + np.array(spectral_labels_Herm_RW)) % 2

        percentage_correct_Herm_RW = max((100 - 100*(np.sum(difference_Herm_RW)/n),100*(np.sum(difference_Herm_RW)/n)))
        print("percentage correct Herm_RW = ", percentage_correct_Herm_RW)


        # DDSYM
        difference_DD_SYM = (np.array(spectral_labels_sort) + np.array(spectral_labels_DD_SYM)) % 2

        percentage_correct_DD_SYM = max((100 - 100*(np.sum(difference_DD_SYM)/n),100*(np.sum(difference_DD_SYM)/n)))
        print("percentage correct DD_SYM = ", percentage_correct_DD_SYM)

        # Herm
        difference_Herm = (np.array(spectral_labels_sort) + np.array(spectral_labels_Herm)) % 2

        percentage_correct_Herm = max((100 - 100*(np.sum(difference_Herm)/n), 100*(np.sum(difference_Herm)/n)))
        print("percentage correct Herm_sparse = ", percentage_correct_Herm)
...
#            plt.savefig('wood_net_trade_flow_per_country_k='+str(n_clusters)+'_' + year + '_top_pair=' + str(-1*top_i) + '.pdf')
            plt.close('all')
...
def degree_sparsifying_sampler(M, A, k, n, C):
    F = np.zeros((M.shape[0], M.shape[1]))
    eigvals, eigvects = np.linalg.eig(A)
    print(eigvals)
    
    # take real part since there is a small imaginary term due to computational innacuracy
    lamda_k = np.sort(np.abs(np.real(eigvals)))[2]


    total_number = 0
    sample_number = 0

    out_degrees = np.sum(M, axis=0)
    in_degrees = np.sum(M, axis=1)
    ...
    # for loop over the rows of M
    for i in range(n):
        # for each element in the row
        for j in range(n):
            # first sample based on out_degree
            degree_u = 5

            pr_out = min(1, M[i,j]*((C * np.log(n))/(out_degrees[i] * lamda_k)))
            pr_in = min(1, M[j,i]*((C * np.log(n))/(in_degrees[j] * lamda_k)))
            
            if M[i,j]!=0:
                total_number += 1

            pr = 0
            if np.random.rand() < pr_out and F[i, j] == 0:
                sample_number += 1
                F[i,j] = M[i,j]/pr_out
            

            if np.random.rand() < pr_in and F[j,i] == 0:
                sample_number += 1
                F[j,i] = M[j,i]/pr_in


    print("percentage of sampled edges:", (sample_number/total_number)*100)
    print('total_number of edges:', total_number)


    return F + np.random.randn(n,n)
...
def normalize(adjacency_matrix):
    ...
    D_sqrt = linalg.fractional_matrix_power(D, -1)
    return np.matmul(D_sqrt, adjacency_matrix) 
...
#        data_file = './process_comtrade/processed_data_TOTAL.csv'
