...
#    axes.text(-195, -45, "$\mathsf{TF}^{\mathsf{norm}}$ = %.2E" % kth_best[0][0], fontsize=100)\
                    #   + "\nCI_vol = " + str(round(kth_best[0][2], 4))) \
#                       + "\nTI_size = " + str(round(kth_best[0][1], 4)), fontsize=100)
...
#            plt.savefig('k_2_sorting_algos.pdf')
            plt.savefig('wood44_net_trade_flow_Herm_RW_k='+str(n_clusters)+'_' + year + '_top_pair=' + str(-1*top_i) + '.pdf')
            plt.close('all')
