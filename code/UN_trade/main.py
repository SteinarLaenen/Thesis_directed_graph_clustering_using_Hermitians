...
#    world.plot(ax=axes[axes_pos[0],axes_pos[1]], color='w')
    world.plot(ax=axes[axes_pos[1]], color='w')
...
#                           ax=axes[axes_pos[0],axes_pos[1]])#, color=('blue', 'yellow', 'grey'))
                           ax=axes[axes_pos[1]])#, color=('blue', 'yellow', 'grey'))
...
#        axes[axes_pos[0],axes_pos[1]].legend(handles=legend_elements)
        axes[axes_pos[1]].legend(handles=legend_elements)
    ...
    axes[axes_pos[1]].text(-190, -90, "TF_norm = %.4E" % kth_best[0][0]\
                    #   + "\nCI_vol = " + str(round(kth_best[0][2], 4))) \
                       + "\nTI_size = " + str(round(kth_best[0][1], 4)))
    axes[axes_pos[1]].set_title(matrix_name)
    axes[axes_pos[1]].get_xaxis().set_visible(False)
    axes[axes_pos[1]].get_yaxis().set_visible(False)
...
def plot_top_clusters():
    """
    """
    for n_clus_vect in [(5,2), (10, 5), (20,10)]:
        runs = 1
    ...
    #        plt.title("legend graph")
            ...
            fig.suptitle('top pair #' + str(-1*top_i) +  ' sorted by TF_norm'  # + to_plot \
                         + ', with k=' + str(n_clusters) + ' (wood HS 44 trade in year = ' + year + ')'
                         , fontsize=30)
...
#            plt.savefig('k_2_sorting_algos.pdf')
            plt.savefig('wood_clustersize_per_countryDD_symHerm_RW_k='+str(n_clusters)+'_' + year + '_top_pair=' + str(-1*top_i) + '.pdf')
            plt.close('all')
